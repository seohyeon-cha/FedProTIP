...
from convs.resnet import reduced_resnet18, resnet18, resnet34, resnet50
from convs.resnet_bn import resnet18_bn
...
from models.alexnet_fot import AlexNet_FOT
from models.vit import vit224
...
def get_convnet(args, pretrained=False):
    ...
    if name == "alexnet":
        ...
        return vit224(pretrained=pretrained, args=args)
    elif name == "resnet32":
        return resnet32()
    elif name == "resnet18":
        return resnet18(pretrained=pretrained, args=args)
    elif name == "resnet18_bn":
        return resnet18_bn(args["n_tasks"], args["n_classes"], nf=32, args=args)
    elif name == "resnet34":
    ...
    elif name == "reduced_resnet18":
        return reduced_resnet18(pretrained=pretrained,args=args)
    else:
        raise NotImplementedError("Unknown type {}".format(name))

def get_vit(args, pretrained):
    name = args["net"].lower()
    if "vit" in name:
        return vit224(pretrained=pretrained, args=args)
...
class IncrementalNet(BaseNet):
    def __init__(self, args, pretrained, gradcam=False):
        ...
        self.fc = torch.nn.ModuleList().to(self.device)
        self.args = args

    def update_fc(self, _cur_task, frz):
        self.generate_fc(self.feature_dim, _cur_task, frz)
 
    def generate_fc(self, in_dim, _cur_task, frz):

        if self.args["classIL"] != "True":
            if _cur_task == 0:
                new_fc = torch.nn.Linear(in_dim, self.args['increment'])
                self.fc.append(new_fc).to(self.device)
        else:      
            if self.args["pretrained"] == "True":
                print("True")
                new_fc = torch.nn.Linear(in_dim, self.args['increment'])
                if "DR" in self.args["method"]:
                    new_fc = torch.nn.Linear(in_dim, self.args['increment'], bias=False)
                self.fc.append(new_fc).to(self.device)
                if _cur_task > 0 and frz == 1:
                    for i in range(_cur_task):
                        for name, param in self.fc[i].named_parameters():
                            param.requires_grad_(False)

            elif self.args["pretrained"] == "False":
                new_fc = torch.nn.Linear(in_dim, self.args['increment'])
                self.fc.append(new_fc).to(self.device)
                if _cur_task > 2 and frz == 1:  # 2 or 0? 
                    for i in range(_cur_task - 2):
                        for name, param in self.fc[i].named_parameters():
                            param.requires_grad_(False)
    ...
    def forward(self, x):
        ...
        out={}
        y = []
        for t in range(len(self.fc)):
            y.append(self.fc[t](x["features"]))
        y = torch.cat(y, dim=1)   
        out['logits'] = y

        if "DR" in self.args["method"]:
            out["orig_featurs"] = x["orig_features"]
...
class Increment_ViT(BaseNet_ViT):
    def __init__(self, args, pretrained, gradcam=False):
        super().__init__(args, pretrained)
        self.gradcam = gradcam
        # self.label_emb = args['label_emb']
    ...
    def generate_fc(self, in_dim, _cur_task, frz):
        ...
        elif self.args["pretrained"] == "False" and _cur_task == 0: 
            new_fc = torch.nn.Linear(in_dim, self.args['increment'] * 3)
            self.fc.append(new_fc).to(self.device)
        else:
            new_fc = torch.nn.Linear(in_dim, self.args['increment'])
            self.fc.append(new_fc).to(self.device)
            
            if _cur_task > 2 and frz == 1:
                for i in range(_cur_task - 2):
                    for name, param in self.fc[i].named_parameters():
                        param.requires_grad_(False)
