...
from utils.data import iCIFAR10, iCIFAR100, CUB200, Imagenet_R, iDomainNet, DomainNet, iImageNet100, iImageNet1000, TinyImageNet200
import torch, copy
...
def _is_student_key(k: str) -> bool:
    # works with/without DataParallel prefix "module."
    return "student." in k or ".student." in k

def average_student_only(state_dicts):
    """Unweighted FedAvg over only student keys."""
    assert len(state_dicts) > 0
    # collect student keys
    student_keys = [k for k in state_dicts[0].keys() if _is_student_key(k)]
    avg = {k: state_dicts[0][k].clone() for k in student_keys}
    for k in student_keys:
        for sd in state_dicts[1:]:
            avg[k] += sd[k]
        avg[k] /= float(len(state_dicts))
    return avg
...
def partition_data(y_train, beta=0.4, n_parties=5):
    ...
    if beta == 0:   # for iid # dividing it regardless of classes right? 
    ...
    def __init__(self, images, labels, trsf, use_path=False):
        ...
        if not isinstance(images[0], np.ndarray):
            for idx in range(len(images)):
                if use_path:
                    trsf_imgs.append(self.trsf(pil_loader(images[idx])))
                else:
                    trsf_imgs.append(self.trsf(Image.fromarray(images[idx])))
            self.images = np.array(trsf_imgs)
...
def _get_idata(dataset_name, args):
    ...
    if name == "cifar10":
        ...
        return CUB200(args, '../data/')
    elif name == "imagenet-r":
        return Imagenet_R(args, '../data/')
    elif name == "domainnet":
        return iDomainNet(args, '../data/')    
    elif name == "dn4il":
        return DomainNet(args, '../data/')
    elif name == "imagenet":
