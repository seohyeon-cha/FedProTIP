...
class Client_base(object):
    def __init__(self, args, idx):
        ...
        self.proto_lr = 0.2 if self.args["dataset"] == "cifar100" else 0.1 
    ...
    def local_training(self, ep_g, model_old):
        ...
        self.model.train()
        self.optimizer = optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.learning_rate,
    ...
    def update_new_set(self):
        if self.signal and self._known_classes != 0: # only updated in the beginning of each task 
    ...
    def entropy_signal(self, loader):
        ...
        print(overall_avg)
        if overall_avg - self.last_entropy > self.thsh: # 1.2로 바꾸기
            res = True
            print(res)
    ...
    def _construct_exemplar_set(self, images, m):
        _, class_mean, feature_extractor_output = self.compute_class_mean(images, self.prev_transform)
        exemplar = []
    ...
    def compute_class_mean(self, images, transform):
        ...
        if self.args["dataset"] == "DomainNet" or "domainnet" in self.args["dataset"]:
            x = x_tt
        x = x.to(self.device)
        ...
        return x, class_mean, feature_extractor_output # x_notrsf prev.code 
    ...
    def prototype_mask(self):
        ...
        iters = 50 if self.args["dataset"] == "cifar100" else 100
        ...
        for label in range(self._known_classes, self._total_classes):
            ...
            if len(idx) > 0:
                ...
                # image.save(f"./run/glfc_perturb/orig_label{label}.png")

        if self.args["classIL"] != "True" and self._cur_task > 0: 
            pass
        else:
        ...
        for i in range(len(proto_labels)):
            ...
            # changed this! 
            opt = optim.SGD([data, ], lr=self.proto_lr, weight_decay=0.00001)
            
            proto_model_w = copy.deepcopy(self.model.state_dict())
            self.proto_model.load_state_dict(proto_model_w)
            # self.proto_model = copy.deepcopy(self.model)
            ...
            # image.save(f"./run/glfc_perturb/label{i}.png")
    ...
    def _compute_loss(self, indexs, imgs, label):
        ...
        # --------- Normal Class-IL path (unchanged) ---------
        output, target = output.to(self.device), target.to(self.device)
    ...
    def efficient_old_class_weight(self, output, label):
        pred = torch.sigmoid(output) # originally 
        ...
        if self._known_classes != 0:
            ...
            index1 = torch.eq(ids, -1).float() # old classes
            index2 = torch.ne(ids, -1).float() # new classes 
